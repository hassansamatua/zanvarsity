-- Safe Content System Creation
-- This script creates the content system without foreign key constraints that might fail

-- ==========================================
-- STEP 1: Create academic_content table without foreign keys first
-- ==========================================
...
-- ==========================================
-- STEP 2: Add faculty column to users table
-- ==========================================

-- Add faculty column to users table if it doesn't exist
ALTER TABLE users ADD COLUMN faculty VARCHAR(20) NULL AFTER role;

-- ==========================================
-- STEP 3: Insert content data
-- ==========================================
...
-- ==========================================
-- STEP 4: Add foreign key constraints (optional - only if tables exist)
-- ==========================================

-- Only add these constraints if you have the proper tables set up
-- Check if users table uses 'id' or 'user_id' as primary key first:
-- DESCRIBE users;

-- If users table has 'user_id' as primary key, use this:
-- ALTER TABLE academic_content ADD CONSTRAINT fk_content_creator FOREIGN KEY (created_by) REFERENCES users(user_id) ON DELETE CASCADE;

-- If users table has 'id' as primary key, use this:
-- ALTER TABLE academic_content ADD CONSTRAINT fk_content_creator FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE CASCADE;
...
-- SELECT * FROM academic_content LIMIT 5;

-- Check users table structure:
-- DESCRIBE users;

-- Count content by type:
-- SELECT unit_type, content_type, COUNT(*) as count FROM academic_content GROUP BY unit_type, content_type;
